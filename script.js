...
function startGame() {
  ...
  socket.on('state', state => {
    ...
    for (const id of Object.keys(state.cars)) {
      ...
      if (!cars[id]) cars[id] = { x: 0, y: 0, h: 0, color: cstate.color };
...
function drawCar(c, isMe) {
  ...
  ctx.fillRect(-h / 4, -w / 4, h / 2, w / 2);
  ctx.fillStyle = '#b71c1c';
  ctx.fillRect(h / 2 - 2, -w / 2, 2, w);
  ctx.fillStyle = '#222';
  ctx.fillRect(2, -w / 2 + 2, h / 3, w - 4);
  ctx.fillStyle = '#111';
  ctx.fillRect(-h / 2 + 3, -w / 2 - 1, 4, 2);
  ctx.fillRect(-h / 2 + 3, w / 2 - 1, 4, 2);
  ctx.fillRect(h / 2 - 7, -w / 2 - 1, 4, 2);
  ctx.fillRect(h / 2 - 7, w / 2 - 1, 4, 2);
  ctx.restore();
}
